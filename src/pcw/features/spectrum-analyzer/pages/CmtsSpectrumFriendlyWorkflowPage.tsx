...
import { ServingGroupSpectrumFriendlyResultsView } from "@/pcw/features/spectrum-analyzer/components/ServingGroupSpectrumFriendlyResultsView";
import {
...
} from "@/pcw/services/servingGroupSpectrumFriendlyService";

const spectrumRoutes = [
  { to: "/spectrum-analyzer/friendly", label: "Friendly" },
] as const;

export function CmtsSpectrumFriendlyWorkflowPage() {
  ...
  const machine = useAdvancedOperationMachine<unknown, unknown>({
    ...
    startOperation: async () => {
      ...
      return startServingGroupSpectrumFriendlyCapture(selectedInstance.baseUrl, requestPayload);
    },
    getStatus: async (operationId: string) => {
      if (!selectedInstance?.baseUrl) {
        throw new Error("No instance selected.");
      }
      const response = await getServingGroupSpectrumFriendlyCaptureStatus(selectedInstance.baseUrl, operationId);
    ...
    stopOperation: async (operationId: string) => {
      ...
      const response = await cancelServingGroupSpectrumFriendlyCapture(selectedInstance.baseUrl, operationId);
      setLatestStatusResponsePayload(response);
      return response;
    },
  });
  ...
  async function loadResults(operationId: string) {
    ...
    setResultsError("");
    try {
      const response = await getServingGroupSpectrumFriendlyResults(selectedInstance.baseUrl, operationId);
      setResultsPayload(response);
      setResultsOperationId(operationId);
      setIsCaptureResultsCollapsed(false);
    } catch (error) {
      setResultsError(error instanceof Error ? error.message : "Failed to load SG Spectrum Analyzer Friendly results.");
    } finally {
  ...
  return (
    <>
      ...
      <Panel
        title={(
          <div className="capture-request-title-layout">
            <FoldablePanelTitle
              id="cmts-spectrum-friendly-capture-request-body"
      ...
      >
        <div id="cmts-spectrum-friendly-capture-request-body" hidden={isCaptureRequestCollapsed}>
          <ServingGroupCaptureRequestForm
      ...
      <Panel
        title={(
          <div className="capture-status-title-layout">
            <FoldablePanelTitle
              id="cmts-spectrum-friendly-capture-status-body"
      ...
      >
        <div id="cmts-spectrum-friendly-capture-status-body" className="operation-status-stack" hidden={isCaptureStatusCollapsed}>
          <div className="operation-status-id-row">
            <label className="field">
              <span>Operation ID</span>
              <input
                id="cmts-spectrum-friendly-operation-id-input"
                name="cmtsSpectrumFriendlyOperationId"
                type="text"
          ...
          {machine.lifecycleState === "starting" || machine.lifecycleState === "running" ? (
            <ThinkingIndicator label="Running SG Spectrum Analyzer Friendly capture and polling status..." />
          ) : null}
      ...
      >
        <div id="cmts-spectrum-friendly-results-body" hidden={isCaptureResultsCollapsed}>
          {isResultsLoading ? <ThinkingIndicator label="Loading SG Spectrum Analyzer Friendly results..." /> : null}
          {resultsError ? <p className="advanced-error-text">{resultsError}</p> : null}
          {!isResultsLoading && !resultsError && resultsPayload ? (
            <ServingGroupSpectrumFriendlyResultsView payload={resultsPayload} />
          ) : null}
          {!isResultsLoading && !resultsError && !resultsPayload ? (
            <p className="panel-copy">Run capture to completion or click Get Results to load SG Spectrum Analyzer Friendly results.</p>
          ) : null}
        </div>
      </Panel>
      <JsonPayloadModal
        id="cmts-spectrum-friendly-request-json-modal"
      ...
      />
      <JsonPayloadModal
        id="cmts-spectrum-friendly-response-json-modal"
